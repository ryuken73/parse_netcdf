...
# 자연스러운 컬러맵 정의 (0mm/h ~ 100mm/h)
colors = [
    (1.0, 1.0, 1.0, 0.0),  # 0mm/h: 투명한 흰색 (결측치)
    (0.0, 0.0, 1.0, 1.0),  # 0mm/h: 청색
    (0.0, 0.5, 0.0, 1.0),  # 10mm/h: 초록색
    (1.0, 1.0, 0.0, 1.0),  # 30mm/h: 노랑색
    (1.0, 0.0, 0.0, 1.0),  # 50mm/h: 빨간색
    (0.5, 0.0, 0.5, 1.0),  # 100mm/h: 자주색
]
n_bins = 256  # 부드러운 전환을 위한 색상 단계 수
colormap_rain = LinearSegmentedColormap.from_list("rain_gradient", colors, N=n_bins)

# 정규화 설정 (0mm/h ~ 150mm/h)
norm = Normalize(vmin=0, vmax=150)  # 최대값 150mm/h로 설정 (데이터 범위에 맞게 조정 가능)
...
cbar = fig.colorbar(im, cax=cax, ticks=[0, 10, 30, 50, 100], label='mm/h')
