...
def process_kim_data(in_dir, tmfc, max_hours, interval, workers):
    config = get_config()
    out_base_path = config.OUT_PATH_KIM
    
    if not out_base_path:
        raise ValueError("config.py에 OUT_PATH_KIM이 설정되지 않았습니다.")
    ...
    print(f"[{tmfc}] 총 {len(valid_files)}개 파일로 최대 {max_hours}시간분 변환 시작 (간격: {interval}분)")
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        for i in range(len(valid_files) - 1):
            ...
            for step_idx in range(steps):
                ...
                # 파일명: g576_v091_easia_etc.2byte_202604030010.png
    ...
    print(f"[{tmfc}] 완료! 총 {global_frames}장의 이미지가 생성되었습니다.")
