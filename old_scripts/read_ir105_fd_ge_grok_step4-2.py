...
# GEOS 좌표를 WGS84 위도/경도로 변환하는 함수 (스케일링 수정)
def latlon_from_geos(Line, Column, sub_lon, coff, cfac, loff, lfac):
    ...
    # 일부 로그만 출력 (처음 5x5 데이터)
    if Line < 50 and Column < 50:  # 처음 50x50 범위 내에서만 로그
        print(f"x = {x}, y = {y} at (Line={Line}, Column={Column})")  # 디버깅 로그
...
# 결과 리스트 초기화
result = []
step = 10  # 샘플링 간격 (필요 시 50으로 늘릴 수 있음, 예: step = 50)
