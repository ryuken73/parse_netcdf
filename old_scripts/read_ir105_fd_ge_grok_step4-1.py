...
# ground_sample_distance 조정 (2km로 가정, 수정된 resolution_factor)
ground_sample_distance = 2000.0  # 자료집에 따른 2km 해상도
resolution_factor = ground_sample_distance / a  # 2km를 라디안으로 변환 (중복 2**16 제거)
print(f"Adjusted ground_sample_distance: {ground_sample_distance} m, resolution_factor: {resolution_factor} rad")

# GEOS 좌표를 WGS84 위도/경도로 변환하는 함수
def latlon_from_geos(Line, Column, sub_lon, coff, cfac, loff, lfac, resolution_factor):
    degtorad = 3.14159265358979 / 180.0
    x = degtorad * ((Column - coff) * 2**16 / cfac) * resolution_factor  # 개선된 스케일링
    y = degtorad * ((dim_y - Line - 1 - loff) * 2**16 / lfac) * resolution_factor  # 행 좌표 뒤집기
...
for y in range(0, dim_y, step):
    for x in range(0, dim_x, step):
        lon, lat = latlon_from_geos(y, x, sub_lon, coff, cfac, loff, lfac, resolution_factor)
        if lon is not None and lat is not None:  # 유효한 좌표만 포함
            if -60 <= lat <= 60 and 80 <= lon <= 180:  # GK-2A 시야 전체
                result.append([
                    float(lon),  # 경도
                    float(lat),  # 위도
...
if lons and lats:
    ...
    ax.set_extent([80, 180, -60, 60], crs=ccrs.PlateCarree())  # GK-2A 시야 전체 범위
