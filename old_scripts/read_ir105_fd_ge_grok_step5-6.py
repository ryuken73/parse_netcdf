...
H = ds.getncattr('nominal_satellite_height') / 1000.0  # 위성 높이 (km 단위로 조정)
...
# GEOS 좌표를 WGS84 위도/경도로 변환하는 함수 (Sd 보정 추가)
def latlon_from_geos(Line, Column, sub_lon, coff, cfac, loff, lfac):
    ...
    Sd = np.sqrt((H * np.cos(x) * np.cos(y))**2 - (np.cos(y)**2 + 1.006739501 * np.sin(y)**2) * (a**2))
    if Sd < 0:  # 무효값 방지 및 디버깅
        print(f"Sd negative: {Sd} at (x={x}, y={y}, Line={Line}, Column={Column})")
        # Sd가 음수일 경우 경계값으로 대체 (임시 보정)
        Sd = 0.0001  # 최소값 설정
