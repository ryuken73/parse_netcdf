...
def latlon_from_geos(Line, Column, sub_lon, coff, cfac, loff, lfac):
    ...
    Sd = np.sqrt((H * np.cos(x) * np.cos(y) / 1000)**2 - (np.cos(y)**2 + 1.006739501 * np.sin(y)**2) * (a**2 / 1000000))
    if Sd < 0:  # 무효값 방지 및 디버깅
        print(f"Sd negative: {Sd} at (x={x}, y={y}, Line={Line}, Column={Column})")
        return None, None
    Sn = (H * np.cos(x) * np.cos(y) / 1000 - Sd) / (np.cos(y)**2 + 1.006739501 * np.sin(y)**2)
    S1 = H / 1000 - (Sn * np.cos(x) * np.cos(y))
...
for y in range(0, dim_y, step):
    for x in range(0, dim_x, step):
        ...
        if lon is not None and lat is not None:  # 유효한 좌표만 포함
            if -80 <= lat <= 80 and -180 <= lon <= 190:  # 범위 유지
                result.append([
                    float(lon),  # 경도
                    float(lat),  # 위도
...
if lons and lats:
    ...
    ax.set_extent([-180, 190, -80, 80], crs=ccrs.PlateCarree())  # 범위 유지
