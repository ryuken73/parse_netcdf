...
# GEOS 좌표를 WGS84 위도/경도로 변환하는 함수 (ground_sample_distance 반영)

def latlon_from_geos(Line, Column, coff, cfac, loff, lfac):
...
for y in range(0, dim_y, step):
    for x in range(0, dim_x, step):
        # lon, lat = geos_to_lonlat(x, y, sub_lon, H, a, b, cfac, lfac, coff, loff, resolution_rad)
        lon, lat = latlon_from_geos(y, x, coff, cfac, loff, lfac)
...
if lons and lats:
  # Cartopy 지도 설정 (Plate Carree 투영, WGS84 기반)
