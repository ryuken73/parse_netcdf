...
def latlon_from_geos(Line, Column, sub_lon, coff, cfac, loff, lfac):
    ...
    if Sd < 0:  # 무효값 방지 및 디버깅
        print(f"Sd negative: {Sd} at (x={x}, y={y})")
        return None, None
...
# 결과 리스트 초기화
result = []
step = 10  # 샘플링 간격 (필요 시 5로 줄일 수 있음)
...
for y in range(0, dim_y, step):
    for x in range(0, dim_x, step):
        ...
        if lon is not None and lat is not None:  # 유효한 좌표만 포함
            if -70 <= lat <= 70 and 60 <= lon <= 190:  # 범위 확장 (태평양 및 극지방 포함)
                result.append([
                    float(lon),  # 경도
                    float(lat),  # 위도
...
if lons and lats:
    ...
    ax.set_extent([60, 190, -70, 70], crs=ccrs.PlateCarree())  # 범위 확장 (태평양 및 극지방 포함)
